...
describe('Curve', function() {
  ...
  it('should work with secp256k1', function() {
    ...
    var p = curve.point('79be667e f9dcbbac 55a06295 ce870b07 029bfcdb 2dce28d9 59f2815b 16f81798', '483ada77 26a3c465 5da4fbfc 0e1108a8 fd17b448 a6855419 9c47d08f fb10d4b8');
    assert(p.validate());
    assert(p.dbl().validate());
    ...
    assert(curve.endo);
    assert.equal(curve.endo.beta.fromRed().toString(16), '7ae96a2b657c07106e64479eac3434e99cf0497512f58995c1396c28719501ee');
    assert.equal(curve.endo.lambda.toString(16), '5363ad4cc05c30e0a5261c028812645a122e22ea20816678df02967c1b23bd72');
    var k = new bn('1234567890123456789012345678901234', 16);
    var split = curve._endoSplit(k);
    assert.equal(split.k1.add(split.k2.mul(curve.endo.lambda)).mod(curve.n).toString(16), k.toString(16));
  });
